...
from app.models import Event, Activity, Enrollment
import secrets
...
class EventService:
    ...
    def create_event(self, owner_username, data):
        ...
        Args:
            owner_username (str): The username of the event creator.
    ...
    def update_event(self, event_id, owner_username, role, data):
        ...
        Args:
            ...
            owner_username (str): Username of the person requesting update.
            role (str): Role of the person requesting update.
            data (dict): New event data.
            
        Returns:
            tuple: (Event, str) - The updated event (or None) and a message.
        """
        ...
        # Replace activities
        # This is a bit simplified; ideally we'd update existing ones.
        # But for this system's scale, clearing and recreating is common.
    ...
    def _create_default_checkin_activity(self, event):
        ...
        activity = Activity(
            ...
            data_atv=event.data_inicio,
            hora_atv=event.hora_inicio,
            carga_horaria=0,
            vagas=-1
        )
    ...
    def _create_activities(self, event, activities_data):
        """Creates multiple activities for 'PADRAO' events."""
        for atv in activities_data:
            activity = Activity(
                ...
                carga_horaria=int(atv.get('horas', 0)),
                vagas=int(atv.get('vagas', -1))
            )
            self.activity_repo.save(activity)
    ...
    def toggle_enrollment(self, user, activity_id, action):
        ...
        Args:
            ...
            activity_id (int): ID of the target activity.
            action (str): 'inscrever' or 'sair'.
            
        Returns:
            tuple: (Enrollment, str) - The enrollment object (or None) and a status message.
        """
    ...
    def list_events_paginated(self, page=1, per_page=10, filters=None):
        """Retrieves a paginated list of events with advanced filtering."""
        query = Event.query
    ...
    def delete_event(self, event_id, owner_username, role):
        """Safely deletes an event if the user has permission."""
        event = self.event_repo.get_by_id(event_id)
    ...
    def get_enrollment(self, activity_id, user_cpf):
        """Retrieves an enrollment by activity and user."""
